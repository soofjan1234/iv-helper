...
## gin框架是什么，有什么好处
一个用 Go 语言编写的轻量级 Web 开发框架

1. 极速性能： Gin 使用了自定义的 HttpRouter（基于前缀树 Radix Tree），它的路由查找速度比标准库快得多。在高并发 API 场景下，这非常关键。
2. 中间件机制（Middleware）： 这是 Gin 的灵魂。你可以轻松地为所有请求添加日志、鉴权、限流、跨域处理，而不需要污染业务逻辑。
3. 参数绑定（Binding）： 它能自动把 JSON、Query 参数、表单数据映射到你的 Go struct 中，并自动校验（利用 validator 库）。
4. 异常处理： Gin 内部对 panic 做了中间件级别的捕获（Recovery），防止单个请求的出错导致整个服务崩溃。

// ----问题----
1. Go的优点是什么
2. Go和Java的区别
3. 讲讲Go的闭包，用途是什么，和函数的区别
4. gin框架是什么，有什么好处
...
| 3 | 2026-03-04 | 3.25 | 2026-03-06 |
| 4 | 2026-03-06 | 3.75 | 2026-03-08 |
| 5 | 2026-03-06 | 3.75 | 2026-03-13 |